    ...
    <div className="bg-background min-h-screen text-on-surface pt-20 font-sans selection:bg-primary selection:text-white">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 services-balanced-grid">
            {servicesList.map((s, i) => (
              <motion.div
                ...
                className={cn("group bg-white border border-outline-variant p-7 md:p-8 rounded-[1.6rem] shadow-sm hover:shadow-xl hover:border-primary/20 transition-all duration-500 flex flex-col items-start min-h-[330px]", i === servicesList.length - 1 ? "lg:col-span-3 lg:max-w-[520px] lg:mx-auto" : "")}
              >
                ...
                <h3 className="text-xl md:text-2xl font-black text-on-surface mb-3 uppercase tracking-tight group-hover:text-primary transition-colors">{s.title}</h3>
                <p className="text-muted leading-relaxed text-sm font-medium mb-7 flex-grow">
                  {s.desc}
                </p>
      ...
      <section className="services-final-cta px-6">
        <div className="services-final-cta-card">
          <div>
            <h2>{isRtl ? 'حلول مهنية تبدأ من احتياج عملك' : 'Professional solutions built around your business needs'}</h2>
            <p>{isRtl ? 'اختر الخدمة المناسبة لمرحلة شركتك، ودع فريق أساس الدقة يساعدك في تنظيم أعمالك المالية والضريبية بثقة ووضوح.' : 'Choose the service that fits your company stage, and let ADBS help organize your financial and tax operations with confidence and clarity.'}</p>
          </div>

          <Link to="/contact" className="services-final-cta-button">
