...
import { Menu, X, ChevronRight, Languages, Home as HomeIcon } from 'lucide-react';
  ...
  return (
    <nav
      className={cn(
        ...
        scrolled 
          ? 'bg-white/95 backdrop-blur-md py-4 shadow-sm border-b border-outline-variant' 
          : 'bg-white/95 backdrop-blur-md py-4 shadow-sm border-b border-outline-variant'
      )}
    >
      <div className="max-w-container-max mx-auto px-6 md:px-12 grid grid-cols-[1fr_auto_1fr] items-center gap-8">
        ...
        <div className="hidden lg:flex items-center justify-end gap-8 justify-self-end">
          ...
          <Link
            to="/contact"
            className="navbar-consultation-btn bg-primary text-white px-8 py-3 text-[19px] font-bold hover:bg-primary-container transition-all tracking-tight uppercase rounded-xl shadow-md hover:shadow-lg active:scale-95"
          >
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="fixed inset-0 bg-background z-[60] flex flex-col p-8 pt-24 gap-10"
          >
            <button
            ...
            <div className="flex flex-col gap-2">
              ...
              {navLinks.map((link) => (
                <Link
                  ...
                  onClick={() => setIsOpen(false)}
                  className={cn(
                    'text-4xl font-noto-serif font-bold py-2 flex justify-center items-center group',
                    location.pathname === link.path ? 'text-primary' : 'text-on-surface/40 hover:text-on-surface'
                  )}
                >
                  {link.id === 'home' ? (
                    <HomeIcon size={34} strokeWidth={2.5} aria-label={link.label} />
                  ) : (
                    link.label
                  )}
            ...
            <div className="mt-auto flex flex-col gap-10">
              <Link
                to="/contact"
                onClick={() => setIsOpen(false)}
                className="bg-primary text-white text-center py-2 text-[19px] font-bold tracking-[0.2em] uppercase rounded-sm"
              >
                {t('services.cta_button')}
              </Link>
              
              <div className="flex items-center justify-center pt-6 border-t border-outline/10">
